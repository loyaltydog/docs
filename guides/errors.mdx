...
Hashed public keys (`ld_live_` / `ld_test_`) get a stable JSON envelope. JWT App Keys and opaque `legacyToken` keep today's body (`detail`). Changing that JWT shape is a regression (SWE-1181).
...
`X-Request-Id` is on every response. Send `X-Request-Id` on the request to echo your own id (1–128 of `A-Za-z0-9._-`).
